...
public class LocalAlwaysFormula<T> implements LocalFormula<T> {
    ...
    public LocalAlwaysFormula(int from, int to, LocalFormula<T> argument) {
    ...
    public LocalFormula<T> next(T state) {
        ...
        LocalFormula<T> nextFormula = new LocalAlwaysFormula<>(Math.max(0, from-1), Math.max(0, to-1), argument);
        if (from>0) {
            return nextFormula;
        }
        return LocalFormula.conjunction(argument.next(state), nextFormula);
    }
    ...
    public boolean equals(Object o) {
        ...
        LocalAlwaysFormula<?> that = (LocalAlwaysFormula<?>) o;
