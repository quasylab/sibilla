...
public class GossipBroadcast {
	...
	private LinkedList<PopulationRule> buildRules() {
		...
		rules.add(new BroadcastRule("AI_spread^*_i", s -> this.lambda_s, AI_INDEX, rg -> PI_INDEX,
				new BroadcastRule.BroadcastReceiver(PS_INDEX,
						(PopulationState s) -> (s.getOccupancy(PS_INDEX) > 0
								? (k / (s.getOccupancy(PU_INDEX) + s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX)))
								: 0.0),
						rgi -> PI_INDEX),
				new BroadcastRule.BroadcastReceiver(PU_INDEX,
						(PopulationState s) -> (s.getOccupancy(PU_INDEX) > 0
								? (k / (s.getOccupancy(PU_INDEX) + s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX)))
								: 0.0),
						rgi -> PI_INDEX),
				new BroadcastRule.BroadcastReceiver(PI_INDEX,
						(PopulationState s) -> (s.getOccupancy(PI_INDEX) > 0
								? (k / (s.getOccupancy(PU_INDEX) + s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX)))
								: 0.0),
						rgi -> PI_INDEX)
		...
		rules.add(new BroadcastRule("AU_spread^*_u", s -> this.lambda_s, AS_INDEX, rg -> PS_INDEX,
				new BroadcastRule.BroadcastReceiver(PI_INDEX,
						(PopulationState s) -> (k
								/ (s.getOccupancy(PU_INDEX) + s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX))),
						rgi -> PS_INDEX),
				new BroadcastRule.BroadcastReceiver(PS_INDEX,
						(PopulationState s) -> (s.getOccupancy(PS_INDEX) > 0
								? (k / (s.getOccupancy(PU_INDEX) + s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX)))
		...
		rules.add(new BroadcastRule("AX_spread^*_u", s -> this.lambda_s, AU_INDEX, rg -> PU_INDEX,
				new BroadcastRule.BroadcastReceiver(PI_INDEX,
						(PopulationState s) -> (k
								/ (s.getOccupancy(PU_INDEX) + s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX))),
						rgi -> PS_INDEX),
	...
	private PopulationState getInitState(int scale) {
		return new PopulationState(new int[] { PI_INIT_SIZE * scale, PS_INIT_SIZE * scale, AI_INIT_SIZE * scale,
				AS_INIT_SIZE * scale, PU_INIT_SIZE * scale, AU_INIT_SIZE * scale });
	}
