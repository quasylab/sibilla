...
public class GossipUnicast {
	...
	public GossipUnicast(double lambda_s, double lambda_a) {
	...
	private LinkedList<PopulationRule> buildRules() {
		...
		rules.add(new UnicastRule(// AI: shuffle_00^ii interaction with PI
				"s_ii",
				(PopulationState s) -> this.lambda_s * (s.getOccupancy(PI_INDEX)
						/ ((s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX) + s.getOccupancy(PU_INDEX)))),
				AI_INDEX, rgi -> PI_INDEX, new UnicastRule.UnicastReceiver(PI_INDEX, s -> 1.0, rgi -> PI_INDEX)));

		rules.add(new UnicastRule(// AI: 0-0 interaction with PI
				"s_iu",
				(PopulationState s) -> this.lambda_s * ((s.getOccupancy(PS_INDEX) + s.getOccupancy(PU_INDEX))
						/ ((s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX) + s.getOccupancy(PU_INDEX)))),
				AI_INDEX, rgi -> PS_INDEX, new UnicastRule.UnicastReceiver(PU_INDEX, s -> 1.0, rgi -> PI_INDEX),
		...
		rules.add(new UnicastRule(// AI: 0-0 interaction with PI
				...
				(PopulationState s) -> this.lambda_s * (s.getOccupancy(PI_INDEX)
						/ ((s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX) + s.getOccupancy(PU_INDEX)))),
				AU_INDEX, rgi -> PI_INDEX, new UnicastRule.UnicastReceiver(PI_INDEX, s -> 1.0, rgi -> PS_INDEX)));

		rules.add(new UnicastRule(// AI: 0-0 interaction with PI
				"s_uu",
				(PopulationState s) -> this.lambda_s * ((s.getOccupancy(PS_INDEX) + s.getOccupancy(PU_INDEX))
						/ ((s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX) + s.getOccupancy(PU_INDEX)))),
				AU_INDEX, rgi -> PS_INDEX, new UnicastRule.UnicastReceiver(PS_INDEX, s -> 1.0, rgi -> PS_INDEX),
		...
		rules.add(new UnicastRule(// AI: 0-0 interaction with PI
				...
				(PopulationState s) -> this.lambda_s * (s.getOccupancy(PI_INDEX)
						/ ((s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX) + s.getOccupancy(PU_INDEX)))),
				AS_INDEX, rgi -> PS_INDEX, new UnicastRule.UnicastReceiver(PI_INDEX, s -> 1.0, rgi -> PS_INDEX)));

		rules.add(new UnicastRule(// AI: 0-0 interaction with PI
				"s_uu",
				(PopulationState s) -> this.lambda_s * ((s.getOccupancy(PS_INDEX) + s.getOccupancy(PU_INDEX))
						/ ((s.getOccupancy(PI_INDEX) + s.getOccupancy(PS_INDEX) + s.getOccupancy(PU_INDEX)))),
				AU_INDEX, rgi -> PU_INDEX, new UnicastRule.UnicastReceiver(PU_INDEX, s -> 1.0, rgi -> PU_INDEX),
				new UnicastRule.UnicastReceiver(PS_INDEX, s -> 1.0, rgi -> PS_INDEX)));

		// END AX_RULES

		rules.add(new BroadcastRule("PU_beact*", s -> this.lambda_a, PS_INDEX, rg -> AS_INDEX));

		rules.add(new BroadcastRule("PI_beact*", s -> this.lambda_a, PI_INDEX, rg -> AI_INDEX));

		rules.add(new BroadcastRule("PX_beact*", s -> this.lambda_a, PU_INDEX, rg -> AU_INDEX));
	...
	public void run(int scale, int iterations, double deadline, int samplings, String outputDir)
		...
		StatisticSampling<PopulationState> aiSamp = getMeasure(samplings, deadline, "AI",
				s -> s.getOccupancy(AI_INDEX));
		StatisticSampling<PopulationState> piSamp = getMeasure(samplings, deadline, "PI",
				s -> s.getOccupancy(PI_INDEX));
		StatisticSampling<PopulationState> auSamp = getMeasure(samplings, deadline, "AU",
				s -> s.getOccupancy(AU_INDEX));
		StatisticSampling<PopulationState> puSamp = getMeasure(samplings, deadline, "PU",
				s -> s.getOccupancy(PU_INDEX));
		StatisticSampling<PopulationState> asSamp = getMeasure(samplings, deadline, "AS",
				s -> s.getOccupancy(AS_INDEX));
		StatisticSampling<PopulationState> psSamp = getMeasure(samplings, deadline, "PS",
				s -> s.getOccupancy(PS_INDEX));
...
//		m.addRules(buildRules());
	...
	private PopulationState getInitState(int scale) {
		return new PopulationState(new int[] { PI_INIT_SIZE * scale, PS_INIT_SIZE * scale, AI_INIT_SIZE * scale,
				AS_INIT_SIZE * scale, AU_INIT_SIZE * scale, PU_INIT_SIZE * scale });
	}
