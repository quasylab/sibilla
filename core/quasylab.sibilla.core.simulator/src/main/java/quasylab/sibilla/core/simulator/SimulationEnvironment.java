...
/**
 * An object responsible for managing simulations. When a new request is received,
 * it is delegated to a {@ling SimulationMangaer} that is built via
...
public class SimulationEnvironment implements Serializable {
	...
	/**
	 * Creates a new simulation environment with default simulation factory. The latter
	 * is a multithreaded based simulation factory.
	 */
	...
	/**
	 * Creates a new simulation environment given a {@link SimulationManagerFactory}. The
	 * latter is used to instantiate the {@link QueuedSimulationManager} used to handle the specific
	 * simulation.
	 *
	...
	/**
	 * Performs a given number of simulations of a given {@link Model} a new set of simulations.
	 * Data are collected via a {@link SamplingFunction}. A monitor is passed to control simulation.
	 *
	...
	public <S extends State> double reachability(
		...
		SimulationManager<S> simulationManager = simulationManagerFactory.getSimulationManager(random, monitor, model.getModelDefinition(), traceConsumer);
	...
	public <S extends State> Trajectory<S> sampleTrajectory(RandomGenerator random, MarkovProcess<S> model, S state, double deadline) {
	...
	public <S extends State> Trajectory<S> sampleTrajectory(RandomGenerator random, MarkovProcess<S> model, S state, double deadline,
										  StatePredicate<? super S> reachPredicate) {
		SimulationUnit<S> unit = new SimulationUnit<S>(model, state,
				SamplePredicate.samplePredicate(deadline,reachPredicate),
				reachPredicate);
