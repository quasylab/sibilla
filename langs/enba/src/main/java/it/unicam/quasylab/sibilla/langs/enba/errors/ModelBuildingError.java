...
package it.unicam.quasylab.sibilla.langs.enba.errors;

import it.unicam.quasylab.sibilla.langs.enba.ExtendedNBAParser;
import it.unicam.quasylab.sibilla.langs.enba.errors.utils.ParseUtil;
...
public class ModelBuildingError {
    ...
    public static ModelBuildingError duplicatedAction(ParserRuleContext action, ParserRuleContext duplicate) {
        return new ModelBuildingError(ParseUtil.getDuplicatedAction(action, duplicate));
    }

    public static ModelBuildingError incompatibleChannel(String species, String channel) {
        return new ModelBuildingError(ParseUtil.getIncompatibleChannel(species, channel));
    }

    public static ModelBuildingError undefinedChannel(String channel, ParserRuleContext context) {
        return new ModelBuildingError(ParseUtil.getUndefinedChannel(channel, context));
    }
    ...
    public static ModelBuildingError illegalUseOfName(String name, ExtendedNBAParser.ReferenceExpressionContext ctx) {
        return new ModelBuildingError(ParseUtil.illegalUseOfName(name, ctx));
    }

    public static ModelBuildingError illegalUseOfName(String name, ExtendedNBAParser.SenderReferenceExpressionContext ctx) {
        return new ModelBuildingError(ParseUtil.illegalUseOfName(name, ctx));
    }
