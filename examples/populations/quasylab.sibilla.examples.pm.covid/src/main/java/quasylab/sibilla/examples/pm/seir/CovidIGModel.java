...
public class CovidIGModel {
    ...
    public static void main(String[] argv) throws FileNotFoundException, InterruptedException, UnknownHostException {
        ...
        StatisticSampling<PopulationState> fagSamp =
                StatisticSampling.measure("Fraction Infected",
                        SAMPLINGS, DEADLINE,
                        s -> (s.getOccupancy(A)+s.getOccupancy(G)+s.getOccupancy(IG))/N) ;
...
//		StatisticSampling<PopulationModel> rSamp = StatisticSampling.measure("#R", SAMPLINGS, DEADLINE, s -> s.getCurrentState().getOccupancy(R)) ;
        ...
        SamplingFunction<PopulationState> sf = new SamplingCollection<>(fsSamp,faSamp,fagSamp,fgSamp,fdSamp,frSamp,figSamp);

        sim.simulate(new DefaultRandomGenerator(),f,initialState(),sf,REPLICA,DEADLINE, true);

        faSamp.printTimeSeries(new PrintStream("data/covid_ig_A_.data"),';');
        fagSamp.printTimeSeries(new PrintStream("data/covid_ig_AG_.data"),';');
        fgSamp.printTimeSeries(new PrintStream("data/covid_ig_G_.data"),';');
        frSamp.printTimeSeries(new PrintStream("data/covid_ig_R_.data"),';');
        fsSamp.printTimeSeries(new PrintStream("data/covid_ig_S_.data"),';');
        fdSamp.printTimeSeries(new PrintStream("data/covid_ig_D_.data"),';');
        figSamp.printTimeSeries(new PrintStream("data/covid_ig_IG_.data"),';');
        System.exit(0);
    }
