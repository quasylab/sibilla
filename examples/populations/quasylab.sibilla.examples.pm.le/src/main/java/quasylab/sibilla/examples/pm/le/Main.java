...
public class Main {
	...
	public static void main(String[] argv) throws InterruptedException, UnknownHostException, IOException {
		...
		PopulationRule rule_C_S0 = new ReactionRule(
				...
				(Function<PopulationState, Double> & Serializable) s -> s.getOccupancy(C)*0.5*SELECT_RATE
		); 
		...
		PopulationRule rule_S0_S1 = new ReactionRule( 
				...
				(Function<PopulationState, Double> & Serializable) s -> (
		...
		PopulationRule rule_S0_L = new ReactionRule( 
				...
				(Function<PopulationState, Double> & Serializable) s -> s.getOccupancy(S0)*WAITING_RATE); 
		...
		PopulationRule rule_S1_L = new ReactionRule( 
				...
				new Specie[] { new Specie(S1) } , 
				new Specie[] { new Specie(L) } , 
				(Function<PopulationState, Double> & Serializable) s -> s.getOccupancy(S1)*WAITING_RATE);
		...
		PopulationRule rule_L_C = new ReactionRule( 
				...
				new Specie[] { new Specie(L) } , 
				new Specie[] { new Specie(C) } , 
				(Function<PopulationState, Double> & Serializable) s -> s.getOccupancy(S0,S1,C)/N*s.getOccupancy(L)*COM_RATE); 
		...
		SimulationEnvironment<PopulationModel,PopulationState> sim = new SimulationEnvironment<>( f, new SequentialSimulationManager<>());
		...
		System.out.println( 
			...
			(Predicate<? super PopulationState> & Serializable) s -> (						
						(s.getOccupancy(L)==1)
						&&
