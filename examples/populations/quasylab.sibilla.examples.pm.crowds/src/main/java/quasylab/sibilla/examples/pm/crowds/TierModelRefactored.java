...
public class TierModelRefactored extends PopulationModelDefinition {
    ...
    public PopulationState state(double... parameters) {
        if (parameters.length != 2) {
            ...
            return new PopulationState(new int[] { (int) parameters[H], (int) parameters[N] });
        }
    }
    ...
    public Model<PopulationState> createModel() {
        ...
        for (int j = 0; j < H; j++) {
            rules.add(new ReactionRule("M1->A" + 0 + j,
                    new Population[] { new Population(reg.indexOf("A", 0, j)), new Population(reg.indexOf("M1")) },
                    new Population[] { new Population(reg.indexOf("AM", 0, j)) }, (t, s) -> LAMBDA_S / H));
        }

        for (int j = 0; j < H; j++) {
            rules.add(new ReactionRule("M2->A" + 0 + j,
        ...
        for (int i = 0; i < N - 1; i++) { // Per ogni tier, tranne l'ultimo
            for (int j = 0; j < H; j++) { // per ogni nodo del tier
                for (int k = 0; k < H; k++) { // per ogni nodo del tier successivo
                    int l = i + 1;
                    rules.add(new ReactionRule("A" + i + j + "->A" + l + k,
                            new Population[] { new Population(reg.indexOf("AM", i, j)),
                                    new Population(reg.indexOf("A", l, k)) },
                            new Population[] { new Population(reg.indexOf("A", i, j)),
                                    new Population(reg.indexOf("AM", l, k)) },
                            (t, s) -> LAMBDA_S / H));
                }
            }
        }
        ...
        for (int i = 0; i < H; i++) {
            int j = N - 1;
            rules.add(
                    new ReactionRule("A" + j + i + "->D", new Population[] { new Population(reg.indexOf("AM", j, i)) },
                            new Population[] { new Population(reg.indexOf("A", j, i)) }, (t, s) -> LAMBDA_S));
        }

        PopulationModel pModel = new PopulationModel(reg.size(), this);
    ...
    public static List<StatisticSampling<PopulationState>> getSamplingList() {
        ...
        for (int i = 0; i < N; i++) {
            int idx = i;
            samplings.add(StatisticSampling.measure("AM" + i, SAMPLINGS, DEADLINE, s -> runningMessages(s, idx)));
            for (int j = 0; j < H; j++) {
                int jdx = j;
        ...
        samplings.add(StatisticSampling.measure("MESSAGES", SAMPLINGS, DEADLINE, TierModelRefactored::runningMessages));
