...
import static it.unicam.quasylab.sibilla.tools.synthesis.CommonForTesting.*;
...
class PSOTaskTest {
    ...
    void testInterpretedFunction(){
        ...
        HyperRectangle searchSpace = new HyperRectangle(
                new ContinuousInterval("x",100,150),
                new ContinuousInterval("y",10,50)
        );

        String objFunctionString = "x*y";
        String constraint1String ="x+4*y>=235";
        String constraint2String ="x+4*y<=245";

        ToDoubleFunction<Map<String,Double>> objFunction = ExpressionInterpreter.getArithmeticExpression(objFunctionString);
        Predicate<Map<String,Double>> constraint1 = ExpressionInterpreter.getPredicateFromExpression(constraint1String);
        Predicate<Map<String,Double>> constraint2 = ExpressionInterpreter.getPredicateFromExpression(constraint2String);
        ...
        Map<String,Double> maximizingValues = new PSOTask().minimize(objFunction,searchSpace,constraints);

        assert(isInRange(maximizingValues.get("x"),100,150));
        assert(isInRange(maximizingValues.get("y"),10,50));
