...
/**
 * Represents a next formula that is satisfied by a local path that in one step
 * satisfies <code>argument</code>.
 *
 * @param argument a local path formula
 */
public record GLoTLLocalPathNextFormula(GLoTLLocalPathFormula argument)
    ...
    public GLoTLLocalPathFormula negate() {
        return new GLoTLLocalPathNextFormula(argument.negate());
    }
}
