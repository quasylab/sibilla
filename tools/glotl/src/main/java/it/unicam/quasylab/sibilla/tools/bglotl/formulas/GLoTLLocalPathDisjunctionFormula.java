...
/**
 * Represents the disjunction formula that is satisfied by a local path that satisfies
 * at least one of its arguments.
 *
 * @param firstArgument a local path formula
 * @param secondArgument a local path formula
 */
public record GLoTLLocalPathDisjunctionFormula(GLoTLLocalPathFormula firstArgument,
    ...
    public <T> GLoTLLocalPathFormula step(T agent, Function<String, Predicate<T>> predicateSolver) {
        return GLoTLLocalPathFormula.disjunction(
    ...
    public GLoTLLocalPathFormula negate() {
        return GLoTLLocalPathFormula.conjunction(firstArgument().negate(),secondArgument().negate());
    }
}
