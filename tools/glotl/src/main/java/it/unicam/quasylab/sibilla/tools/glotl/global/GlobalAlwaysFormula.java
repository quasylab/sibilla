...
public class GlobalAlwaysFormula<T, S extends IndexedState<T>> implements GlobalFormula<T, S> {
    ...
    public GlobalAlwaysFormula(int from, int to, GlobalFormula<T, S> argument) {
    ...
    public GlobalFormula<T,S> next(S state) {
        ...
        GlobalFormula<T,S> nextFormula = new GlobalAlwaysFormula<>(Math.max(0, from-1), Math.max(0, to-1), argument);
        if (from>0) {
            return nextFormula;
        }
        return GlobalFormula.conjunction(argument.next(state), nextFormula);
    }
    ...
    public boolean equals(Object o) {
        ...
        GlobalAlwaysFormula<?, ?> that = (GlobalAlwaysFormula<?, ?>) o;
